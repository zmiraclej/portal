(function($) {
	...
	var noCloseModal = false;
	$.myUtil = {
		alert: {
			fn: function(){},
			show: function(title, content, config, fn) {
				var cfg = {};
				$.extend(cfg, config);
				noCloseModal = cfg.noCloseModal;
		...
		dialogFrame: {
			show: function(src, config) {
				...
				if ($('.mu-dialog-frame', baseDoc).length == 0) {
					var html = '<div class="mu-dialog-frame mu-hide"><iframe src="" id="dialogFrame"' 
						+ ' name="dialogFrame" width="100%" height="' + config.height 
						+ '" scrolling="false" frameborder="0"></iframe></div>';
					$('body', baseDoc).append(html);
				}
		...
		setDialogHistory: function(val) {
			$.myUtil.setCookie("dialogHistory", val, 1);
		},
		addDialogHistory: function(val) {
			$.myUtil.setDialogHistory($.myUtil.getDialogHistory() + val);
		},
		getDialogHistory: function() {
			return parseInt($.myUtil.getCookie("dialogHistory")) || 0;
		}
	};
	$(function() {
