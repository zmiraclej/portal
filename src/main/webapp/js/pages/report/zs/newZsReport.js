...
var startDate = ''; // 开始时间
var endDate = ''; // 结束时间
var scheduleType = $("#schedule").val(); // 招生类型
var compareToCode = '';
//定义图表加载接口
var url = "";
...
layui.use(['jquery', 'form', 'layedit', 'laydate', 'element'],
	function() {
		...
		//选择招生类别
		form.on('select(schedule)', function(data) {
			scheduleType = data.value;
			reportLoginMenu();
		});
		//选择对比条件
...
function reportLoginMenu() {
	...
	if(reportName == "历年报名数据分析" || reportName == "") {
		echartType = 1;
		url = "./report/zs/searchEnrollHistoryByItem";
	}
	if(reportName == "录取数据分析") {
		echartType = 1;
		url = "./report/zs/searchEnrollDetailByItem";
	}
	if(reportName == "招生类别分析") {
		echartType = 2;
		url = "./report/zs/searchStudentTypeInfo";
	}
	if(reportName == "招生情况分析") {
		echartType = 1;
		url = "./report/zs/searchZhaoshengInfo";
	}
	if(reportName == "生源地分析") {
		echartType = 1;
		url = "";
